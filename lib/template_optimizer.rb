...
class TemplateOptimizer
  ...
  module ERB_Helpers
    ...
    def local_assign_pattern?(ast)
      local_assigns = [:lvar, :local_assigns]
      if ast[0] == :if
        stmt = ast[2] if method_call?(ast[1], :has_key?) == local_assigns
      else
        stmt = ast
      end
      return false unless stmt
      assigned_var, assigned_exp = lasgn?(stmt)
      return false unless assigned_var && assigned_exp
      expected_exp = method_call(local_assigns, :[], [:lit, assigned_var])
      if assigned_exp == expected_exp
        assigned_var
      end
    end
